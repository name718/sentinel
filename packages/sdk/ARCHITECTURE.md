...
| 数据类型 | 压缩策略 | 限制 |
|---------|---------|------|
| resources | 只保留关键字段，截断 URL | 最多 50 条 |
| longTasks | 保留完整数据 | 最多 20 条 |
| breadcrumbs | 保留完整数据 | 最多 20 条 |
| sessionReplay.events | 保留完整数据 | 最多 500 条 |
| message | 截断过长消息 | 最多 1000 字符 |
| stack | 截断过长堆栈 | 最多 5000 字符 |
...
interface MonitorConfig {
  ...
  useWorker?: boolean;      // 使用 Web Worker 上报，默认 true（自动降级）
}
```
